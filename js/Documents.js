...
export function DocumentsPage() {
    ...
    function buttons() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (let index = 0; index < dataModified.length; index++) {
                        createInboundCard(index, dataModified[index]);
                    }
                },
        ...
        function ShowOutbounds() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    // Loop through the table data
                    for (const entry of data.data) {
                        const { out_id, out_adress, out_comment, g_quantity, g_name } = entry;

                        if (!blendedData[out_id]) {
                            blendedData[out_id] = {
                                out_id, out_adress, out_comment, g_quantity, g_name,
                                goods: []
                            };
                        }
                        blendedData[out_id].goods.push({ g_quantity, g_name });
                    }
                    ...
                    console.log(dataModified.length);
                    for (let index = 0; index < dataModified.length; index++) {
                        createOutboundCard(index, dataModified[index]);
                    }
                },
                error: function (xhr, status, error) {
                    console.error('Error fetching data:', error);
        ...
        function createInboundCard(index, data) {
        ...
        function createOutboundCard(index, data) {
            // Create the main container
            const $cardContainer = $('<div>').addClass('card-container');
